...
import { MessageFromApi, ON_RAMP, GET_DEPTH, CANCEL_ORDER, CREATE_ORDER, GET_OPEN_ORDERS, } from "../types/fromApi";
...
export class Engine {
    ...
    constructor() {
        ...
        if (snapshot) {
            ...
            this.orderbooks = snapshotSnap.orderbooks.map((o: any) => new Orderbook(o.baseAsset, o.bids, o.asks, o.lastTradeId, o.currentPrice));
    ...
    saveSnapshot() {
        ...
        fs.writeFileSync('./snapshot.json', JSON.stringify(snapshotSnap));
    }
    ...
    createOrder(market: string, price: string, quantity: string, side: "buy" | "sell", userId: string) {
        ...
        this.checkAndLockFunds(baseAsset, quoteAsset, side, userId, price, quoteAsset, quantity);
        ...
        fills.forEach(fill => {
            RedisManager.getInstance().pushMessage({
                ...
                data: {
                    ...
                    isBuyerMarker: fill.otherUserId === userId,  //Check if the id are write
                    price: fill.price,
                    quantity: fill.price,
        ...
        fills.forEach(fill => {
            RedisManager.getInstance().publishMessage(`trade@${market}`, {
                ...
                data: {
                    e: "trade",
                    t: fill.tradeId,
                    m: fill.otherUserId === userId,   //Check if the id are write
                    p: fill.price,
                    q: fill.qty.toString(),
                    s: market,
                }
    ...
    updateBalance(userId:string, baseAsset:string, quoteAsset: string, side:"buy"|"sell",fills:Fill[], executedQty:number){
        if (side=== "buy") {
            fills.forEach(fill=>{
                ...
                // @ts-ignore
                this.balances.get(userId)[quoteAsset].locked= this.balances.get(fill.otherUserId)?.[quoteAsset].available+ (fill.qty*fill.price);
